...
export class ClientsController extends CommonControllerConfig {
    ...
    //create client
    create = async (req: CustomClientRequest, res: Response): Promise<void> => {
        ...
        try {
            ...
            if (error) res.send(this.s('failed', error.details[0].message, 406));
            else {
                const userExist = await Client.findOne({ where: { email } });

                // checking if client exists
                if (userExist) res.send(this.s('failed', 'email already exists', 409));
                else {
                ...
                try {
                    ...
                    if (error) res.send(this.s('failed', error.details[0].message, 409));
                        ...
                        const updatedClient = await Client.update(
                            ...
                            { where: { id: req.params.clientId }, returning: true },
                        );
    ...
    delete = async (req: Request, res: Response): Promise<void> => {
        try {
            ...
            if (deletedUser === 1) res.send(this.s('success', 'client deleted'));
            else res.send(this.s('failed', 'client does not exists, may be already deleted or was not created', 409));
        } catch (e) {
