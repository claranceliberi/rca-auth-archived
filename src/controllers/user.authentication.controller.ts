...
export class UserAuthenticationController extends CommonControllerConfig {
        ...
        try {
            if (!user)
                ...
                if (truePassword) {
                    ...
                    const jwt = this.generatesAccessToken({
                        email: user.email,
                        id: user.id,
                    });
    ...
    authorizeApp = async (req: Request, res: Response): Promise<void> => {
        ...
        try {
            ...
            if (!app) {
            } else res.send(this.s('failed', 'app does not exists', 409));
        } catch (e) {
            res.send(this.s('failed', e, 500));
        }
    };
